...
(function (domGlobals) {
    ...
    var getRetainStyleProps = function (editor) {
        return editor.getParam('paste_retain_style_properties','color font-size background background-color');
    };
    var getWordValidElements = function (editor) {
        var defaultValidElements = '-strong/b,-em/i,-u,-span,-p,-img[src|alt|width|height|wordsrc],-ol,-ul,-li,-h1,-h2,-h3,-h4,-h5,-h6,' + '-p/div,-a[href|name],sub,sup,strike,br,del,table[width],tr,' + 'td[colspan|rowspan|width],th[colspan|rowspan|width],thead,tfoot,tbody';
    ...
    var getPasteDataImages = function (editor) {
        return editor.getParam('paste_data_images', true, 'boolean');
    };
    ...
    var getUploadImage = function (editor) {
        return editor.getParam('paste_upload_image' , null);
    };
    ...
    function filterStyles(editor, validStyles, node, styleValue) {
        ...
        global$4.each(styles, function (value, name) {
            ...
            if (getRetainStyleProps(editor) === 'all' || validStyles && validStyles[name]) {
                outputStyles[name] = transUnit(value);
            }
        });
    ...
    var filterWordContent = function (editor, content) {
        ...
        domParser.addNodeFilter('img' , function (nodes) {
            var i = nodes.length, node, src, word_src;
            var $URI = editor.baseURI
            while(i--){
                node = nodes[i];
                src = node.attr('src');
                word_src = null;
                if (src && src.indexOf('file://') === 0) {
                    word_src = src;
                    //src = $URI.directory + '/plugins/pasteword/img/spacer.gif';
                }
                if(word_src){
                    node.attr({
                        src:src,
                        class:'wordsrc',
                        wordsrc:word_src,
                        /*style: 'background:url('+$URI.directory+'/plugins/pasteword/img/localimage.png) no-repeat center center;border:1px  solid #ddd;'*/
                    })
                }
            }
        });
    ...
    var doPaste = function (editor, content, internal, pasteAsText) {
        ...
        var uploadImage = getUploadImage(editor);
        if(/wordsrc/.test(args.content) && uploadImage && isFunction(uploadImage) && args.cancelled === false){
            var div = document.createElement('div');
            div.innerHTML = args.content;
            var imgs = div.querySelectorAll('img.wordsrc');
            var imgsrc = [];
            for (var i=0;i<imgs.length;i++)
            {
                imgsrc.push(imgs[i].src/*.replace('file:///' , '')*/)
            }
            var files = [];
            var index = 0;

            editor.windowManager.openUrl({
                title: '处理Word图片上传', // The dialog's title - displayed in the dialog header
                url: editor.baseURI.directory + '/plugins/pasteword/paste.html?url='+encodeURIComponent(imgsrc.join(',')),
                width: 640,
                height:320,
                onMessage:function (dialogApi , details) {

                    var cmd = details.data;
                    if(cmd.cmd == 'file'){
                        var file = cmd.file;
                        var filename = file.name;
                        index++;
                        console.log(uploadImage)
                        uploadImage(editor , file , function (filepath) {
                            global$4.each(imgs , function (img , key) {
                                if(basename(img.src) == filename){
                                    img.setAttribute('src' , filepath);
                                    img.classList.remove('wordsrc');
                                }
                            })
                            index--;
                            console.log(index)
                ...
                onClose:function () {
                    insertContent(editor, div.innerHTML , pasteAsText);
                }
            })
        }else{
    ...
    var readBlobsAsDataUris = function (editor,items) {
        var uploadImage = getUploadImage(editor);

        return global$3.all(map(items, function (item) {
            return new global$3(function (resolve,reject) {
                if(uploadImage && isFunction(uploadImage)){
                    var blob = item.getAsFile ? item.getAsFile() : item;
                    uploadImage(editor , item , function ( filepath ) {
                        resolve({
                            blob: blob,
                            file:true,
                            uri: filepath
    ...
    var pasteImageData = function (editor, e, rng) {
        ...
        if (getPasteDataImages(editor) && dataTransfer) {
            ...
            if (images.length > 0) {
                e.preventDefault();
                readBlobsAsDataUris(editor,images).then(function (blobResults) {
                    if (rng) {
    ...
    function register$3(editor) {
        //console.log(editor)
        editor.contentCSS.push(editor.baseURI.directory+'/plugins/pasteword/css/theme.css');
    }
